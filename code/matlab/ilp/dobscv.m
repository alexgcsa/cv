...
for class=classes
    ...
    while ~isempty(i)
        ...
        distance = sum((x2(:, i) - x2(:, i(1))).^2);  % Fast squared Euclidean distance
%         distance = pdist2(x(i, :), x(i(1), :), 'cosine');  % Slow, but it is easy to change the used distance
        [~, i2] = sort(distance);
