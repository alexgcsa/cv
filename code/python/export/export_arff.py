...
def export_fold(data_set_name):
    # 1) Load arff
    arff = open(path + 'data/arff/' + data_set_name + '.arff', 'r', encoding='ISO-8859-1').read()
    ...
    # 5) Zip the folds
    filename = path + 'export/iterative_order_2_deflated/' + data_set_name + '-Stratified10FoldsCV-Mulan.zip'
    xmlfile = path + 'data/arff/' + data_set_name + '.xml'
    ...
    with ZipFile(filename, 'w', compression=ZIP_DEFLATED, compresslevel=9) as zipObj:
        ...
        zipObj.write(xmlfile, arcname=data_set_name + '.xml')
